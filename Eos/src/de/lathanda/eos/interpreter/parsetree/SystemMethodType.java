...
public class SystemMethodType implements MethodType {
     ...
     */
    public SystemMethodType(SystemType target, SystemType[] parameters, Type ret, String name, String originalName) throws NoSuchMethodException {
		...
		this.signature = ReservedVariables.createSignature(originalName, parameters.length);
	}
	...
	public void compile(ArrayList<Command> ops, Expression target, boolean autowindow) throws Exception {
		...
		ops.add(new de.lathanda.eos.interpreter.commands.Method(getParameters(), method));
    }
}
