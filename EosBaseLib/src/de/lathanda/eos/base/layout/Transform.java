...
public class Transform {
    ...
    public Transform scalePositionAt(double x, double y, double factor) {
    	return new Transform(
    			v.substract(x, y).multiply(factor).add(x, y),
    			angle, mirrorx, scale
    	);
    }
    ...
    public Transform transform(Transform child) {
        Transform result = new Transform(
                child.v.multiply(scale).rotate(angle).add(v),
                child.angle,
                child.mirrorx,
                child.scale
        );
    ...
    public Transform transformBack(Transform child) {
        ...
        return new Transform(
                result.v.substract(v).rotate(-angle).multiply(1/scale),
                result.angle,
                result.mirrorx,
                result.scale
        );
